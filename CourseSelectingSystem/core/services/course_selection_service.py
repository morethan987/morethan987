...
from core.exceptions.exceptions import CourseNotAvailableException, ElementNotFoundException
import logging
...
class CourseSelectionService:
    ...
    def select_course_general(self, sb, course_name: str) -> bool:
        ...
        try:
            ...
            checkboxes = sb.find_elements('css selector', 'div.ant-drawer-body table input.ant-checkbox-input[type="checkbox"]')
            ...
            if not checkbox_clicked:
                self.logger.warning(f"No available checkbox found for course: {course_name}")
                return False
    ...
    def select_course_with_teacher(self, sb, course_name: str, teacher_name: str) -> bool:
        """
        ...
        try:
            ...
            # Find teacher row
            row, is_full, checkbox = self.element_locator.find_teacher_row(sb, teacher_name)

            if row is None:
                self.logger.warning(f"Teacher {teacher_name} not found for course {course_name}")
                self.element_locator.close_sidebar(sb)
                return False

            if is_full:
                self.logger.info(f"Course {course_name} with teacher {teacher_name} is full")
                self.element_locator.close_sidebar(sb)
                return False
            ...
            # Click checkbox
            sb.driver.execute_script("arguments[0].click();", checkbox)
            self.logger.info(f"Clicked checkbox for course {course_name}, teacher {teacher_name}")
    ...
    def _complete_selection_process(self, sb, course_name: str, teacher_name: str = None) -> bool:
        ...
        try:
            ...
            if selection_btn:
                sb.driver.execute_script("arguments[0].scrollIntoView();", selection_btn)
                time.sleep(0.2)
                selection_btn.click()

                course_info = f"{course_name}" + (f" (Teacher: {teacher_name})" if teacher_name else "")
                self.logger.info(f"Clicked selection button for: {course_info}")
            else:
                self.logger.error(f"Selection button not found for course: {course_name}")
                return False
            ...
            course_info = f"{course_name}" + (f" (Teacher: {teacher_name})" if teacher_name else "")
            self.logger.error(f"Error completing selection process for {course_info}: {e}")
            return False
